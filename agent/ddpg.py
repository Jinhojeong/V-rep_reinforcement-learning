...
class DDPG(object):
    ...
    def __init__(self,config):
        ...
        self.var_init=tf.global_variables_initializer()
        # update critic
        y=self.reward+tf.multiply(self.target_q,self.done)
        q_loss=tf.pow(self.critic_net.out_-y,2)/config.batch_size+ \
    ...
    def update(self,state0,action0,reward,state1):
        ...
        self.sess.run([self.update_critic,self.update_actor], \
